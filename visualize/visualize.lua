...
local function main()
   for itr = 1, dataset:size(1) do
      ...
      winImg = image.display{image = seqOfFrames,
                             ...
                             nrow = 20, win = winImg}
      for l = 1, L do
         image.display{image = errorImg[l], legend = 'Error layer ' .. l, nrow = 20, zoom = 2^(l-1)}
         image.display{image = RImg[l], legend = 'Representation layer ' .. l, nrow = 20, zoom = 2^(l-1)}
      end
      io.read()
   end
end
