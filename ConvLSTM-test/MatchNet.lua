...
-- NOTE: having a parametrized layer version is hard to do and to debug.
-- for now I will use a fixed params version below. 
--
-- The code for the parametrized version is after commented out, and only tested for 1-2 layers
...
function mNet(nlayers, input_stride, poolsize, mapss, clOpt, testing)
   ...
   if nlayers == 1 then
      ...
      cR = nn.SpatialConvolution(mapss[1], mapss[2], 3, 3, input_stride, input_stride, 1, 1) -- same layer E
...
-- this version below is the one with parametrized layers:
