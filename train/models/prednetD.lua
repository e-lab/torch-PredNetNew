...
--------------------------------------------------------------------------------
   ...
   for l = L, 1, -1 do
      if l == L then
         ...
         upR = tempR[l+1] - nn.SpatialUpSamplingNearest(2)     -- Upsample prev Rl+1
      end
      --  R = {upR, inputs[3*l + 2]} - nn.JoinTable(1, 3)

      local lstm = ({upR, inputs[3*l], inputs[3*l+1]}   --  local lstm = ({R, inputs[3*l], inputs[3*l+1]}
              - convLSTM:getModel(channels[l+1], channels[l], lstmLayer))
                        :annotate{name = 'LSTM ' .. l,
                                  graphAttributes = gaR}

      --  outputs[3*l-1] 
      tempC[l] = (lstm - nn.SelectTable(1)) -- Cell State
      --  outputs[3*l]   
      tempR[l] = (lstm - nn.SelectTable(2)) -- Hidden state
      outputs[3*l+1] = inputs[3*l+2]
   end
...
--------------------------------------------------------------------------------
   for l = 1, L do
      ...
      if l == 1 then          -- First layer block has E and Ah as output
         --                img,        Rl/Hl
         local E_Ah = ({inputs[1], tempR[l]}
                      - block(l, L, oChannel, oChannel, vis))
                      :annotate{name = '{E,Ah}'..l..'[t]', graphAttributes = gaE}

         local E, Ah = E_Ah:split(2)

         local lstm = {E, tempC[l], tempR[l]}   --  local lstm = ({R, inputs[3*l], inputs[3*l+1]}
              - convLSTM:getModel(2*channels[l], channels[l], lstmLayer)
         outputs[3*l-1] = (lstm - nn.SelectTable(1)) -- C
         outputs[3*l] = (lstm - nn.SelectTable(2))   -- R
