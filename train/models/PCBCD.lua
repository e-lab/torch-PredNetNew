...
local function block(l, L, iC, oC, vis)
   ...
   local nodeAh = nn.Sequential()
                  :add(nn.SpatialFullConvolution(oC, iC, 3, 3, 2, 2, 1, 1, 1, 1))
                  :add(nn.ReLU())
...
--------------------------------------------------------------------------------
   ...
   for l = L, 1, -1 do
      if l == L then upR = inputs[2] else upR = Rtemp[l+1] end
      c.upR = channels[l+1]
      --  E = inputs[3*l + 2]
      c.E = channels[l]
      R = inputs[3*l + 1]
